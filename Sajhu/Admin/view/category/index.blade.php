                    ...
                    <table id="datatable" class="table table-striped table-bordered">
                        ...
                        @if(isset($categories) && count($categories)>0)
                            @foreach($categories as $category)
                                <tr>
                                    <td class="col-md-5">{{ $category->category_title }}</td>
                                    <td class="col-md-3">
                                        <a href="{{ route(AppHelper::getBaseRoute('edit'),$category) }}"
                                        ...
                                        @if($category->have_child)
                                            <a href="{{ route('admin.subcategory.index',$category) }}"
                                               class="btn btn-dark btn-xs"><i class="fa fa-list"></i> Sub Category </a>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
